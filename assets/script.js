...
//mettre le fetch
function mainFunc(lat, long, code, latitude, longitude, ville) {

  // donner les paramètres fonctions afficher météo redéfinir, ailleurs en dehors fonction
...
async function chercherVille_func() {
  ...
  try {
    ...
    // Afficher la ville dans le HTML
    const container = document.querySelector(".position-user");
    ...
    ville_elem.querySelector(".longitude").textContent = longitude.toFixed(2);
    ville_elem.querySelector(".altitude").textContent = latitude.toFixed(2);
    container.appendChild(ville_elem);

    // Mettre à jour le nom de la ville principale
    const villeNomElem = document.querySelector(".ville-nom");
    if (villeNomElem) villeNomElem.textContent = `${name}, ${country}`;
